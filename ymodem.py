...
class YMODEM(object):
    ...
    # initialize
    def __init__(self, getc, putc, mode='ymodem128', header_pad=b'\x00', pad=b'\x1a'):
        self.getc = getc
        self.putc = putc
        self.mode = mode
    ...
    def send(self, file_stream, file_name, file_size=0, retry=20, timeout=15, callback=None, flash_status_callback=None):
        try:
            packet_size = dict(
                ymodem=1024,
                ymodem128=128
        ...
        current_packet = 0  # 当前数据包编号
        total_packet = math.ceil(file_size / packet_size)  # 总数据包数量
        ...
        print('data_for_send: ', data_for_send)
        self.log.info("<<< Packet 0 >>> " + str(len(data_for_send)))
        # self.sent_data_size = int(0)
        # self.sent_percentage = int(self.sent_data_size/(file_size/100))
        error_count = 0
        cancel = 0
        ...
        while True:
            ...
            if char:
                if char == ACK:
                    self.log.info("<<< ACK")
                    # self.sent_data_size += len(data_for_send)
                    char2 = self.getc(1)
                    if char2 == CRC:
                        self.log.info("<<< CRC")
                        break
                    else:
                        self.log.warning(">>> ACK wasn't CRC")
                        break
                elif char == CAN:
                    self.log.info("<<< CAN")
                    if cancel:
            ...
            while True:
                ...
                if char == ACK:
                    ...
                    if callback:
                        current_packet += 1
                        try:
                            sent_percentage = math.ceil((current_packet / total_packet) * 100)
                        except ZeroDivisionError:
                            sent_percentage = 100
                        callback(sent_percentage)
                        print('<<< sent_percentage1: ', sent_percentage)
                        self.log.info("<<< Percentage Start")
                    error_count = 0
                    break
            ...
            if char == ACK:
                if callback:
                    ...
                    try:
                        ...
                        sent_percentage = 100
                    callback(sent_percentage)
                    print('<<< sent_percentage2: ', sent_percentage)
                break
            else:
                error_count += 1
                if error_count > retry:
